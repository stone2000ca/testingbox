...
Extract all factual data from the parent's message. Return ONLY valid JSON. Do NOT explain.`;
    ...
    try {
      let invokeResult = await base44.integrations.Core.InvokeLLM({
        prompt: systemPrompt + '\n\n' + userPrompt,
        model: 'gpt-5',
        response_json_schema: extractionSchema
      });
      if (typeof invokeResult === 'string') {
        try { invokeResult = JSON.parse(invokeResult); } catch { invokeResult = {}; }
      }
      result = invokeResult || {};
      intentSignal = result?.intentSignal || 'continue';
      console.log('[INTENT SIGNAL] via InvokeLLM (primary)', intentSignal);
    } catch (invokeLLMError) {
      console.error('[EXTRACT] InvokeLLM failed, falling back to callOpenRouter:', invokeLLMError.message);
      try {
        ...
        intentSignal = result?.intentSignal || 'continue';
        console.log('[INTENT SIGNAL] via callOpenRouter (fallback)', intentSignal);
      } catch (fallbackError) {
        console.error('[EXTRACT FALLBACK] callOpenRouter also failed:', fallbackError.message);
        result = {};
